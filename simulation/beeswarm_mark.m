function x = beeswarm_mark(x,y,varargin)
% This code is originally by Ian Stevenson, CC-BY 2019
% It has some modifications by Brian Camley, CC-BY 2020
% 
...
addOptional(p,'MarkType',{'o','o','o','o','o','o','o','o','o,'})
...
if isfinite(p.Results.dot_size)
    ...
    for i=1:length(ux)
        if isempty(p.Results.MarkerFaceColor')
            scatter(x(ic==i),y(ic==i),p.Results.dot_size*36,'filled','MarkerFaceAlpha',p.Results.MarkerFaceAlpha,'MarkerEdgeColor',p.Results.MarkerEdgeColor,'MarkerFaceColor',cmap(i,:),'Marker',p.Results.MarkType{i})
        else
        ...
        switch lower(p.Results.overlay_style)
            ...
            case 'ci'
                ...
                line([ux(i)-2*rwid ux(i)+2*rwid],[1 1]*mean(yorig(ic==i)),'LineWidth',3,'Color',cmap(i,:))
            case 'se'
                line([1 1]*ux(i),mean(yorig(ic==i))+[-1 1]*std(yorig(ic==i))/sqrt(sum(ic==i)),'Color',cmap(i,:),'LineWidth',2)
                line([ux(i)-4*rwid ux(i)+4*rwid],[1 1]*mean(yorig(ic==i)),'LineWidth',3,'Color',cmap(i,:))
        end
